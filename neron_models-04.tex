...
\begin{Korollar}\label{thm:neronmausweierstrassgl}
  ...
  $R$.
  \begin{proof}
    Nach \ref{thm:neronmodelllokal} können wir uns wieder auf den
    lokalen Fall $W\times_R R_s$ beschränken für die geschlossenen
    Punkte $s\in\Spec(R)$. $W\times_R R_s$ ist wieder eigentliches,
    glattes Weierstraßmodell von $E_\K$ über $R_s$ und daher nach
    \ref{thm:erweiterunggruppenstruktur} ein $R_s$-Gruppenschema, das
    die Gruppenstruktur von $E_\K$ erweitert. 
    % TODO!!! KAPUTT
    
    $W\times_R R_s$ von $W$ über den Lokalisierungen
    $R_s$ von $R$ für die geschlossenen Punkte $s\in\Spec(R)$.
    ...
    Gruppenverknüpfung und genauso Inverses und Einheitsschnitt lassen
    sich jeweils wie 
    Diese 
    verkleben nach Definition bzw. wegen der Separiertheit von $W$ zu
    einer Gruppenschemastruktur auf ganz $W$, da sie durch die gleiche
    Abbildung zwischen graduierten Ringen definiert werden, sprich auf
    dem generischen Punkt übereinstimmen. Vergleiche auch
