...
\begin{Satz}
  ...
  \item\cite[Proposition IV.4.5(b)]{silverman2} Ist $g\geq1$ kann
    ein minimales eigentliches reguläres Modell $X_{\text{min}}$ für $C$
    gewählt werden.
    D.\,h. für jedes andere eigentliche reguläres Modell $X$ für
  ...
  \item\cite[Proposition IV.4.6]{silverman2}
    Für ein minimales, eigentliches, reguläres Modell $X_{\text{min}}$
    für $C/\K$ gilt, dass jeder $\K$"=Automorphismus von $C$ sich zu
    einem $R$-Morphismus von $X_{\text{min}}$ erweitert, der glatte
...
\begin{Lemma}
  ...
  \begin{proof}
    Wir zeigen erst, dass $C\times_R R'$ eine arithmetische Fläche
    über $R'$ ist.
    Projektiv, flach und integral sind stabil unter Basiswechsel.
    % ??? Regularität?
    % TODO!!
    % Regularität bleibt unter dem Faserprodukt mit einem regulären
    % Schema erhalten: Wähle für einen Punkt $x$ im Faserprodukt eine
    % affine, offene Umgebung $\Spec(A)\times_R\Spec(B)=\Spec(A\otimes
    % B)$, dann hat der Halm $\O_{A\otimes B, x}=(A\otimes
    % B)_x=(A_x\otimes B_x)$ in $x$
  \end{proof}
\end{Lemma}  
