...
\begin{Korollar}\label{thm:allgmorphismuserweiterung}
  ...
  \begin{proof}
    ...
    nach \autoref{thm:erweindeutig} alle Erweiterungen von 
...
\begin{proof}[Beweis von \autoref{thm:neronmodelllokal}]
  ...
  $\pi\colon\Spec(A)\to\Spec(R)$ eine $R$-Algebra mit
  $\pi(\m)=s$. Dann faktorisiert $\pi$ über
  $\Spec(A)\to\Spec(R_s)\to\Spec(R)$ und für jeden Morphismus
  $\Spec(\Quot(A))\to X$ faktorisiert eine Erweiterung
  $\Spec(A)\to X$ über $\Spec(A)\to X\times_R\Spec(\O_{R,s})$, wo sie
  eindeutig ist wegen Separiertheit von $X\times_R\Spec(\O_{R,s})$
  nach Voraussetzung.

  Für Glattheit auf $X$ benötigen wir nur noch, dass $X$ flaches
  $R$-Schema ist, da $R$ noethersch und $X$ nach Voraussetzung von
  endlichem Typ ist, also endlich repräsentiert.
  Sei $x\in X_s\subset X$, $s\in\Spec(R)$. Dann ist der Halm
  $\O_{X,x}=\O_{X\times_R\O_{R,s},x}$, welcher wegen Glattheit von
  $X\times_R\O_{R,s}$ flach über $R_s$ ist.
  ...
  nach \autoref{thm:erweindeutig} auf dem Schnitt ihrer
  ...
  Daher liefert Verkleben einen der eindeutigen $u_s$ einen
  eindeutig bestimmten Morphismus $f\colon Y\to X$, der
  $f_\K|_{Y\cap Y_\K}$ erweitert.
