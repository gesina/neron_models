...
\begin{Lemma}\label{thm:eigweierstrassmodelle}
  Ein Weierstraßmodell von $E_\K$ über $R$ ist irreduzibel und lokal
  endlich repräsentiert.
  \begin{proof}
    \begin{description}
    \item[irreduzibel]
...
\begin{Lemma}\label{thm:glattüberdvr}
  ...
  irreduzibles
  % ??? irreduzibel oder integral? (s. Hartshorne, III.9.7)
  $R$-Schema lokal endlichen Typs (äquivalent: lokal endlich
  repräsentiert \cite[Remark 10.36]{wedhorn}) mit nichtleerer,
  generischer Faser $X_\K$.
  ...
  \begin{proof}
    ...
    Seien also die Fasern von $X$ glatt und insb. reduziert.
    ...
    Es gilt also, Flachheit zu zeigen.
...
\begin{Lemma}\label{thm:weierstrassglatt}
  ...
  \begin{proof}
    ...
    \item $W$ ist irreduzibles, lokal endlich repräsentiertes
      ...
      glatt ist.
    \item Für $\K$-Schemata lokal endlichen Typs sind glatt und
      geometrisch regulär äquivalent \cite[Corollary 6.32]{wedhorn},
      da $\K$ perfekt ist, ist dies sogar äquivalent zu regulär
      \cite[Remark 6.33]{wedhorn}.
      Schemata lokal endlichen Typs sind genau dann regulär, wenn sie
      in allen geschlossenen Punkten regulär sind
      ...
      sind.
      Damit ist es nach \autoref{thm:glattüberdvr} glatt über $R$
...
\begin{Bemerkung}\label{thm:weierstraßmodellglatt}
  Für den Fall eines diskreten Bewertungsringes $R$ liefert
  ...
  \begin{enumerate}[label=(\roman*)]
  \item $0\neq\Delta_{(\pi)}=\bar\Delta\in R$.
  \item $\Delta\not\in(\pi)$ bzw. $\pi\nmid\Delta$.
  \item $\ord_R(\Delta)=0$ bzw. $\Delta\in R^\times$
  \end{enumerate}
  Letztere ist einfach nachzuprüfen.
\end{Bemerkung}
...
\begin{Lemma}\label{thm:erweiterunggruppenstruktur}
  ...
  % ???
  Sei $W$ ein Weierstraßmodell von $E_\K$ über $R$.
  Das offene $R$-Unterschema $W^0$ der glatten Punkte von $W$ ist
  ebenfalls $R$-Modell von $E_\K$
  und ein glattes $R$"=Gruppenschema mit Addition bzw. Inversenbildung
  \begin{gather*}
    \mu_0\colon W^0\times_R W^0 \longto W^0
    \qquad \text{bzw.}\qquad
    i_0\colon W^0\longto W^0
  \end{gather*}
  welche die Gruppenstruktur (Addition $\mu$ und Inversenbildung $i$)
  von $E_\K$ als abelsche Varietät erweitern
...
\begin{Korollar}\label{thm:neronmausweierstrassgl}
  ...
  Koeffizienten im Dedekindring $R$ ($\K$ perfekt) und
  $W\subset\P_R^2$ sei das durch diese Gleichung erzeugte projektive
  $R$-Schema.
