...
\begin{Lemma}\label{thm:erwrrational}
  ...
  generischen Fasern.
  Dann erweitert sich $\phi_\K$ zu einer $R$-rationalen Abbildung
  $\phi\colon Y\to W$.
  \begin{proof}
...
\begin{Satz}\label{thm:egruppenschema}
  ...
  \begin{proof}
    \begin{Lemma}
      ...
      \begin{proof}
        Wie in \autoref{thm:fallstriktehenselisierung}
        % TODO!! ausformulieren
        erweitert sich $\mu_\K\colon E_\K\times_\K E_\K\to E_\K$
        auf eine $R$-rationale Abbildung $\mu\colon E\dra E$.
        Für eine normale Komposition bleiben Assoziativität und Inverse zu
        zeigen.
        \paragraph{Assoziativität} Für Assoziativität müssen für alle $x, z\in
        E$, so dass es wohldefiniert ist, die folgenden $R$-rationalen
        Abbildungen gleich sein (also gleich auf der gemeinsamen
        Definitionsmenge)
        \begin{gather*}
          \mu(\mu(x,\bullet),z), \mu(x,\mu(\bullet,z))
          \colon E \dra E
        \end{gather*}
        Nun sind aber beide die eindeutige Erweiterung von
        \begin{gather*}
        ...
        Für $R$-Rationalität von $\phi\colon (x,y)\mapsto (x,\mu(x,y))$
        (bzw. $\psi$) muss noch gezeigt werden, dass $\Dom(\phi)$
        (bzw. $\Dom(\psi)$) dicht in der speziellen Faser von $E$ ist,
        also in jedem generischen Punkt der speziellen Faser definiert
        ist.
