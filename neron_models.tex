...
\begin{Lemma}\label{thm:morphismuserweiterung}
  ...
  \begin{proof}
    ...
    Für die Eindeutigkeit beachte, dass $f$ eindeutig auf $D(f)$ ist,
    falls die Lokalisierung $A_f\to A_y$ injektiv ist. Dies gilt für
    $Y$ integral.
  \end{proof}
\end{Lemma}

\begin{Korollar}\label{thm:allgmorphismuserweiterung}
  Sei $S$ lokal noethersches Schema, $\pi\colon Y\to S$ und $X$
  $S$-Schemata, $X$ endlichen Typs, $Y$ integral, $s\in S$,
  ...
  \begin{proof}
    ...
    für ein $s'\in\Spec\O_{S,s}$. $\pi$ induziert einen
    ...
    \autoref{thm:morphismuserweiterung} auf genügend kleinen
    Umgebungen $U(y)\subset Y$ von $y$ eindeutig zu Morphismen. Wegen
    der Eindeutigkeit stimmen diese auf den Überschneidungen
    überein und lassen sich zu einem eindeutigen Morphismus
    \begin{gather*}
      f\colon U=\bigcup_{y\in Y\times_S\O_{S,s}} U(y)\to X
    \end{gather*}
    verkleben.
  \end{proof}
\end{Korollar}
...
\begin{proof}[Beweis von \autoref{thm:neronmodelllokal}]
  ...
  Betrachte nun die irreduziblen Komponenten $Y_i$ von $Y$, welche
  integral sind, nachdem $Y$ glatt also reduziert ist.
  Nach \autoref{thm:allgmorphismuserweiterung} gibt es eine offene
  Umgebung $U_i(s)$ von $Y_i\times_R\O_{R,s}$, auf der
  $u_s|_{Y_i\times_R\O_{R,s}}$ wiederum
  eindeutig auf einen Morphismus $u_i(s)\colon U(s)\to X$ erweitert
  wird.
  Die $u_i(s)$ stimmen per Konstruktion auf dem Schnitt ihrer
  Definitionsbereiche überein, die ganz $Y_i$ überdecken
  ...
  Daher liefert Verkleben einen der eindeutigen $u_i(s)$ jeweils einen
  eindeutig bestimmten Morphismus $u_i\colon Y_i\to X$, der
  $u_\K|_{Y_i\cap Y_\K}$ erweitert.
  
  Die $u_i$ wiederum stimmen wegen der Eindeutigkeit
  % TODO!! Eindeutigkeit???!
  auf dem Schnitt
  ihrer Definitionsbereiche überein und verkleben eindeutig zu einem
  $u\colon Y\to X$.
...
\begin{Lemma}\label{thm:erwrrational}
  ...
  \begin{proof}
    ...
    Für die Eindeutigkeit sei $Y$ reduziert, $\phi$ gegeben und
    $x\in\Dom(\phi)$.
    ...
    In einem entsprechenden irreduziblen geschlossenen Unterschema
    $V_x$ von $Y$ ist $\dim_{V_x}(x)=1$. Nachdem $Y$ reduziert ist,
    ist $V_x$ integral und $\Spec(\O_{V_x,x})=\{x,y\}$ ist
    Bewertungsring, wobei $y\coloneqq (0)\in\Spec(\O_{V_x,x})$ ein Punkt der
    Dimension $(n-1)$ in $Y$ ist und $\Quot(O_{V_x,x})=\k(y)$.
    Nach dem Bewertungskriterium für Eigentlichkeit von $W$ ist
...
\begin{Definition}
  ...
  \item\cite[Proposition IV.4.5(b)]{silverman2} Ist $g\geq1$ kann
    ...
    ist die nach \autoref{thm:erwrrational} induzierte birationale
...
\begin{Satz}\label{thm:fallstriktehenselisierung}
  ...
  \begin{proof}
    ...
    % präsentiert wird.
    Nach \autoref{thm:erwrrational} erweitert sich $\phi_\K$ auf eine
