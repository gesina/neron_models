...
\hyphenation{Né-ron=Ab-bil-dungs-ei-gen-schaft}
...
\begin{Satz}
  ...
  \begin{proof}
    ...
    \item
      ...
      $\K'$-Morphismus. 
      \begin{description}
        % Da $\phi_{\K'}$ ein $\K'$-Morphismus ist,
        % kommutiert das folgende Diagramm
        % \begin{center}
        %   \begin{tikzcd}
        %     Y_{\K'}'\arrow[r, equal]
        %     & Y'\times_{R'}\K'
        %     \arrow[r,"{\phi_{\K'}}"]\arrow[dr, "{\proj_{\K'}}"{below}]
        %     & X_{R'}\times_{R'}\K'\arrow[r,equal]\arrow[d,"{\proj_{\K'}}"]
        %     &X_{\K'}\\
        %     &&\Spec(\K')
        %   \end{tikzcd}
        % \end{center}
        % Damit ist
        % $\phi_{\K'}=\left(
        %   \proj_{X_{R'}}\circ\phi_{\K'} \;,\; \proj_{\K'}\circ\phi_{\K'}
        % \right)
        % =\left(
        %   \proj_{X_{R'}}\circ\phi_{\K'} \;,\; \proj_{\K'}
        % \right)$
        % und bereits eindeutig bestimmt durch
        % \begin{gather*}
        %   \phi_{\K'} =
        %   \left(
        %     \proj_{X_{\K}}\circ\phi_{\K'} \;,\; \proj_{\K'}
        %     \right)
        %   \colon
        %   Y_{\K'}'\longto
        %   X_{\K'}=X_{\K}\times_{R}\Spec(\K')
        % \end{gather*}
        % Genauso jeder Lift
        % \begin{gather*}
        %   \phi_{R'} =
        %   (\proj_{X}\circ\phi_{R'}) \times \proj_{R'}
        %   \colon
        %   Y'\longto
        %   X_{R'}=X\times_{R}\Spec(R')
        % \end{gather*}
        % Für den $\K$-Morphismus $(\proj_{X_{\K}}\circ\phi_{\K'}')\colon
        % Y_{\K'}'\to X_{\K}$ gilt die Eindeutigkeit der
        % Néron-Abbildungseigenschaft, weshalb
        % $(\proj_{X}\circ\phi_{R'})$ und damit $\phi_{R'}$ eindeutig
        % bestimmt ist.
      \item[Existenz] 
        ...
        \end{gather*}
        mit der Néron-Abbildungseigenschaft zu einem eindeutigen
        $R$-Morphismus
        \begin{gather*}
...
\begin{Satz}\label{thm:neronmodelllokal}
  ...
  \item $X\times_R\Spec(\O{S,s})$ ist Néron-Modell seiner generischen
...
\begin{Bemerkung}\label{thm:gruppenschemaerweiterung}
  ...
  Néron-Abbildungseigenschaft eindeutig auf eine
...
\begin{Bemerkung}\label{nerongruppenschemaglatt}
  ...
  Néron-Abbildungseigenschaft erfüllt, so ist $X$ bereits separiert,
  ...
  \begin{proof}[Beweisskizze]
    ...
    Dieser erweitert wegen der Néron-Abbildungseigenschaft eindeutig
...
\begin{Bemerkung}[Spezialfall elliptische Kurven]
  ...
  Dann ist ein Néron-Modell von $X_\K$ nach
  \ref{thm:gruppenschemaerweiterung} und \ref{nerongruppenschemaglatt} ein
  glattes $R$-Modell und Gruppenschema $X/R$ mit der
...
\begin{Satz}
  Sei $\Spec(R)$ zusammenhängend und $A_\K$ abelsche Varietät über
  $\K$ mit guter Reduktion (d.\,h. es gibt ein glattes, eigentliches
  $R$-Modell $A$ von $A_\K$).
  ...
  \begin{proof}[Beweisskizze]
    ...
    \begin{description}
    \item[$A\cong X$] $X$ ist als flaches $R$-Schema mit
      zusammenhängender generischer Faser $A_\K$ zusammenhängend.
      % ???
      Die Néron-Abbildungseigenschaft von $X$ liefert
...
Sei $R$ ein Dedekindring, $\K=\Quot(R)$ und $X/\K$
