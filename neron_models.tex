...
\begin{Bemerkung}\label{thm:dichtefaser}
  ...
  rationale Abbildung (vgl. \autoref{def:ratabb})
  \begin{gather*}
    \phi\colon X\dra E
  \end{gather*}
...
\begin{Satz}\label{thm:erweindeutig}
  Sei $R$ Dedekindring, $Y$ und $X$ $R$-Schemata, $X$ separiert.
  Dann ist ein Morphismus $\phi\colon Y\to X$ eindeutig durch seine
  Einschränkung auf die generischen Punkte von $Y$ bestimmt.
  Insbesondere sind, falls $Y$ flach über $X$ ist, Erweiterungen von
  Morphismen der generischen Fasern eindeutig, falls sie existieren.
  \begin{proof}
    Sei $x\in\Dom(\phi)$.
...
\begin{Lemma}\label{thm:morphismuserweiterung}
  ...
  $f_y$
  % TODO!! eindeutig???
  von einem Morphismus $f\colon U\to X$ erweitert wird.
  % Ist $Y$ integral, kann $U$ so gewählt werden, dass $f$ auf $U$
  % eindeutig ist.
  \begin{proof}
    ...
    \end{gather*}
    $D(f)$ ist eine offene Umgebung von $y$, womit die Existenz
    gezeigt ist.

    % Für die Eindeutigkeit beachte, dass $f$ genau dann eindeutig auf
    % $D(f)$ ist, wenn die Lokalisierung $A_f\to A_y$ injektiv
    % ist. Dies gilt für $Y$ integral.
  \end{proof}
\end{Lemma}

\begin{Korollar}\label{thm:allgmorphismuserweiterung}
  Sei $S$ lokal noethersches Schema, $\pi\colon Y\to S$ integeres und
  flaches $S$-Schema,
  $X$ separiertes $S$-Schemata endlichen Typs,
  $s\in S$,
  $f_s\colon Y\times_S\O_{S,s}\to X\times_S\O_{S,s}$ ein
  $S$-Morphismus.
  ...
  \begin{proof}
    ...
    \begin{gather*}
      f_s(y)\colon \Spec\O_{Y,y}\longto X\times_S\O_{S,s}\subset X
      \qquad \forall y \in Y\times_S\O_{S,s}
    \end{gather*}
    Diese erweitern sich jeweils nach
    \autoref{thm:morphismuserweiterung} auf offenen
    ...
    U(y')\subset Y$, $y,y'\in Y$, sind offene, irreduzible
    Unterschemata mit demselben generischen Punkt $\eta$ wie $Y$.
    Da $\eta\in Y\times_S\O_{S,s}$, sind nach
    \autoref{thm:erweindeutig} wegen Separiertheit von $X$ alle
    Erweiterungen von $f_s$ auf $U(y)$ und $U(y)\cap U(y')$ eindeutig.
    Insbesondere sind die $f_s(y)$ eindeutig und stimmen auf den
...
\begin{proof}[Beweis von \autoref{thm:neronmodelllokal}]
  ...
  generischen Fasern.
  Als glattes $R$-Schema ist $Y$ reduziert und insbesondere regulär,
  weshalb die irreduziblen Komponenten alle offen und disjunkt
  sind. Wir können also \OE{} annehmen, dass $Y$ integer ist.
  ...
  $u_s|_{Y\times_R\O_{R,s}}$ wiederum
  eindeutig auf einen Morphismus $u(s)\colon U(s)\to X$ erweitert
  wird.
  Die $u(s)$ stimmen wieder wegen Separiertheit von $X$
  % TODO: Warum ist X hier separiert?
  nach
  \autoref{thm:erweindeutig} auf dem Schnitt ihrer
  ...
  Daher liefert Verkleben einen der eindeutigen $u(s)$ einen
  eindeutig bestimmten Morphismus $u\colon Y\to X$, der
  $u_\K|_{Y\cap Y_\K}$ erweitert.
...
\begin{Korollar}\label{thm:neronmausweierstrassgl}
  ...
  \begin{proof}
    ...
    Morphismus.
    In \autoref{thm:erwrrational} zeigen wir, dass $\phi_\K$ eine
    $R$-rationale Abbildung $\phi\colon Y\dra W$ liefert.
...
\begin{Lemma}\label{thm:erwrrational}
  ...
  generischen Fasern.
  Dann erweitert sich $\phi_\K$ zu einer
  $R$-rationalen Abbildung $\phi\colon Y\dra W$.
  Ist $Y$ reduziert, ist $\phi$ eindeutig.
  % TODO: QUATSCH! Reduziert hier nicht notwendig?
  \begin{proof}
    ...
    % % $Y$ reduziert,
    % % TODO: BLÖDSINN
    % $\phi$ gegeben und $x\in\Dom(\phi)$.
    % Wir werden induktiv nach der Dimension von $x$ vorgehen. Für
    % $\dim_Y(x)=0$ liegt $x$, da $Y$ flach ist, in der generischen
    % Faser und $\phi$ ist hier nach Voraussetzung eindeutig.
    % Sei $\phi$ eindeutig definiert in $y\in\Dom(\phi)$ für
    % $\dim_Y(y)=(n-1)$ und sei $\dim_Y(x)=n$.
    % In einem entsprechenden irreduziblen geschlossenen, reduzierten
    % Unterschema $V_x$ von $Y$ ist $\dim_{V_x}(x)=1$.
    % % Nachdem $Y$ reduziert ist, ist
    % % TODO: BLÖDSINN!!
    % $V_x$ ist integral und
    % $\Spec(\O_{V_x,x})=\{x,y\}\subset\Spec(\O_{Y,x})$ ist
    % Bewertungsring, wobei $y\coloneqq (0)\in\Spec(\O_{V_x,x})$ ein Punkt der
    % Dimension $(n-1)$ in $Y$ ist mit $y\in\Dom(\phi)$ und
    % $\Quot(\O_{V_x,x})=\k(y)$.
    % Nach dem Bewertungskriterium für Separiertheit von $W$ ist
    % $\phi|_{\Spec(\O_{V_x,x})}$ die eindeutige Erweiterung von
    % $\phi|_{\Spec(\k(y))}$. Da $\phi$ auf $y$ bereits eindeutig war,
    % ist es auch eindeutig auf $x$.
  \end{proof}
 \end{Lemma}
...
\begin{Definition}
  ...
  \item\cite[Proposition IV.4.5(a)]{silverman2}
    ...
    (kurz: eigentliches Modell für $C/\K$).
  \item\cite[Proposition IV.4.5(b)]{silverman2} Ist $g\geq1$ kann
    ein minimales eigentliches Modell $X_{\text{min}}$ für $C$
    gewählt werden.
    D.\,h. für jedes andere eigentliche Modell $X$ für
    $C/\K$ und jeden Isomorphismus $\phi_\K$ der generischen Fasern
    ist die nach \autoref{thm:erwrrational}
    % TODO: integer fehlt als Voraussetzung
    induzierte birationale
    Abbildung $\phi\colon X\dra X_{\text{min}}$ ein $R$-Morphismus.
  \end{enumerate}
\end{Definition}
...
\begin{Satz}\label{thm:eigminmodell}
  ...
  \item\optcite[Proposition IV.4.6]{silverman2}
    Für ein minimales Modell $X_{\text{min}}$
    für $C/\K$ gilt, dass jeder $\K$"=Automorphismus von $C$ sich zu
  ...
  \begin{proof}
    ...
    \item Die Eigenschaften flach, projektiv und integer sind stabil
      ...
      Basiswechsel
      % TODO!!! Warum ist regulär stabil unter glattem Basiswechsel?
      und die Minimalitätseigenschaft von $X_{\text{min}}\times_R R'$
...
\begin{Lemma}\label{thm:schnittearithmflglatt}
  ...
  $k$, $\K$, $E_\K$, $C$, $E$ wie in \autoref{thm:exneronmodelle},
  ...
  % TODO: Voraussetzungen anpassen; regulär stabil unter glattem Basiswechsel?
  \begin{proof}
    ...
    \autoref{thm:eigminmodell} wieder eine arithmetische Fläche über
    ...
    \autoref{thm:arithflschnittbilderglatt}.
...
\begin{Satz}\label{thm:fallstriktehenselisierung}
  ...
  \begin{proof}
    ...
    Da $E$ $R$-Gruppenschema, $Y$ glatt und $\phi$ eine
      ...
      Kodimension 1 nach \autoref{thm:rationalzumorphismus}.
    ...
    \autoref{thm:morphismuserweiterung} besagt, dass sich
    ...
    Nun ist aber $\eta\in E_\K$, also
      ...
      \autoref{thm:schnittearithmflglatt}
  ...
  \begin{proof}
    \begin{Lemma}
      ...
      \begin{proof}
        ...
        \autoref{thm:erwrrational} eindeutig zu einer $R$-rationalen
        ...
        Dimension 1, sind also diskrete Bewertungsringe, da $E\times_R
        E$ glatt und somit regulär ist. Nach
        \autoref{thm:schnittearithmflglatt} liegt dann
        ...
        % TODO: integer statt reduziert
        \autoref{thm:erwrrational} eindeutige Erweiterung von
        \begin{gather*}
        ...
        Restklassenkörper. Wegen \autoref{eq:stalkdimension} hat $R'$
        Dimension 1, ist demnach ein diskreter Bewertungsring und
        $C_{R'}\coloneqq C\times_R R'$ nach \autoref{thm:eigminmodell}
        ...
        \begin{gather*}
          ...
          E_{\K'}
        \end{gather*}
        Nach \autoref{thm:eigminmodell} erweitert sich $\tau_{\K'}$ zu
    ...
    \begin{Lemma}
      Sei nach \autoref{thm:weil} $G$ das zu $E$ $R$-birationale
      Gruppenschema. Dann ist $G$ isomorph zu $E$.
      \begin{proof}
        …
      \end{proof}
    \end{Lemma}
    Dies beendet den Beweis zu \autoref{thm:egruppenschema}.
  \end{proof}
\end{Satz}
