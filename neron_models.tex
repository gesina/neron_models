...
\begin{Satz}\label{thm:eigminmodell}
  ...
  \item\cite[Proposition IV.4.6]{silverman2}
    ...
    einem $R$-Automorphismus von $X_{\text{min}}$ erweitert, der glatte
    Punkte auf glatte Punkte abbildet.
  ...
  \begin{proof}
    ...
    \item Die Minimalitätseigenschaft liefert zu einem
      ...
      $\tau\colon X_{\text{min}}\to X_{\text{min}}$.
...
\begin{Lemma}
  ...
  Sei $R'$ diskreter Bewertungsring und flache $R$-Algebra
  % ??? Welche Eig. benötigt?
  % TODO!!
  und $\phi\colon \Spec(R')\to C$ ein $R$-Morphismus.
  Ist $\phi((0))\in E$, gilt $\im(\phi)\subset E$.
  \begin{proof}
    $C\times_R R'$ ist wie in \autoref{thm:eigminmodell} wieder eine
    arithmetische Fläche über $R'$.
    …
  \end{proof}
\end{Lemma}  
...
\begin{Satz}\label{thm:egruppenschema}
  Sei $R$ strikt henselscher diskreter Bewertungsring (mit perfekten
  Quotientenkörpern) und $\K$, $E_\K$, $C$, $E$ wieder wie in
  \autoref{thm:exneronmodelle}.
...
\begin{Lemma}
  ...
  \begin{proof}
    ...
    zeigen.
    \begin{description}
    \item[Assoziativität] Für Assoziativität müssen für alle $x, z\in
    ...
    \item[Inverses $R$-rational]
      ...
      …
      % \autoref{thm:eigminmodell}: stabil unter Basiswechsel
