  ...
  \begin{proof}
    ...
    \item Eigenschaften henselscher Ringe auf Schnitteinschränkung:
      ...
      \item Falls $R$ strikt henselsch, ist $X(R)\to X_k(k)$ surjektiv
        ...
        % ??? Wie kann X_k(k) dicht in X_k sein?
      \end{enumerate}
...
\begin{Definition}[Henselisierung]\label{def:henselisierung}
  ...
  lokaler Ring $\Rh$ mit einem lokalen Homomorphismus $i\colon
  R\to\Rh$, s.\,d. folgende universelle Eigenschaft erfüllt ist:
  ...
  Homomorphismus $u^\text{h}\colon R^h\to A$, so dass das folgende Diagramm
  kommutiert
  \begin{center}
    \begin{tikzcd}
      R \arrow[r, "{u}"]\arrow[d, "{i}"]
      & A \\
      \Rh\arrow[ur, "{u^\text{h}}"]
    \end{tikzcd}
  \end{center}
...
\begin{Definition}[strikte Henselisierung]\label{def:striktehenselisierung}
  Die strikte Henselisierung eines lokalen Rings $R$ ist …
\end{Definition}
\begin{Satz}
