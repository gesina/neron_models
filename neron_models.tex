...
% Um von einer Erweiterung der Gruppenstruktur sprechen zu können,
% sollte man auch fordern, dass $E(Y)$ eine Erweiterung von $E_\K(Y_\K)$
% ist ($Y_\K$ generische Faser von $Y$), in anderen Worten:
% Interessant wird das Verhältnis der Gruppen $E_\K(Y_\K)$ zu $E(Y)$
% Da auch auf diesen Gruppen von einer schönen Erweiterung ausgehen zu
% können, braucht es weiterhin die Forderung:
% Nachdem ein wichtiger Bestandteil der Gruppenstruktur von $E_\K$ ist,
% dass die Menge der Schnitte $E_\K(Y_\K)$ eines $\K$-Schemas $Y_\K$
% eine Gruppe ist, ergibt sich eine weitere Forderung an $E$:
\begin{quote}
  Für alle \enquote{gutartigen} (hier: glatten) $R$-Schemata $Y$ mit
  generischer Faser $Y_\K$, soll $E_\K(Y_\K)\hookrightarrow E(Y)$
  Untergruppe sein.
\end{quote}
% Als Diagramm für einen Schnitt in $E_\K(\K)$:
% \begin{center}
%   \begin{tikzcd}
%     Y_\K\arrow[r]\arrow[d, hook]&E_\K\arrow[d, hook]\\
%     Y\arrow[r, dashed, "\exists!"]&E
%   \end{tikzcd}
% \end{center}
...
regulären Fall sogar das Néron-Modell von $E_\K$ ist.
...
Kurve über $R$ zu erhalten. Solche \enquote{gutartigen} Kurven über
...
liegt.
\begin{Satz}[Weil]\label{thm:weil}
...
\begin{Definition}[minimale Weierstraßgleichung]
  ...
  ist.
\end{Definition}
Es stellt sich heraus, dass sich die Bewertung der Diskriminante in
einer endliche Anzahl an Fällen klassifizieren lässt.
Dies ist interessant für die Klassifizierung der speziellen Fasern von
Néron-Modellen elliptischer Kurven, wie wir später sehen werden.
% TODO: Sieht man die Klassifizierung später?
...
\begin{Satz}\label{thm:exneronmodelle}
  ...
  Nachdem minimale eigentliche reguläre Modelle elliptischer
...
\section{Gruppenstruktur hinreichend}
...
\chapter{Beispiele und Ausblick}
...
nur die Weierstraßgleichung der elliptischen Kurve benötigt, diese
