...
\begin{Lemma}\label{thm:weierstrassglatt}
  Sei $R$ diskreter Bewertungsring, $\K=\Quot(R)$, $E_\K$ elliptische
  Kurve über $\K$, $W$ ein Weierstraßmodell von $E_\K$ über $R$.
  $W$ ist genau dann glatt, wenn die spezielle Faser von $W$ glatt
  ist.
  ...
  \begin{proof}
    ...
    Ist $W$ glatt, so auch jede Faser, da Glattheit stabil unter
    Basiswechsel ist.

    Sei also die spezielle Faser von $W$ glatt und insb. reduziert.
    Dann ist $W$ reduziert, da die zwei Fasern von $W$ nach
    Voraussetzung reduziert sind.
    Außerdem ist $W$ nach \autoref{thm:eigweierstrassmodelle} 
    irreduzibel, daher integral, und lokal endlich präsentiert.
    Wir wissen, dass $W$ glatt ist, wenn es lokal endlich präsentiert
    ...
    Das zeigt, dass $W$ flach ist \cite[Proposition III.9.7]{hartshorne}.
    ...
    aber der Abschluss $\overline{\{\eta\}}=W$ von $\eta$ ist darin
